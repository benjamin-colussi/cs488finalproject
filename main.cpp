...
int main(const int argc, const char* argv[]) {
    ...
    if (objLoadSucceed) {
        ...
        if (RENDER_CORNELL_BOX) {
            ...
            // lighting
            Material light; light.type = LIGHT; light.emission = float3(100.0);
            Sphere cornellBoxLight = Sphere(float3(-0.25f, 0.25f, -0.25f), 0.025f, light); // back top corner
            if (ATMOSPHERIC_SCATTERING) cornellBoxLight = Sphere(float3(0.0f, 0.4f, 0.0f), 0.05f, light); // centre ceiling for fog
            globalScene.addLight(&cornellBoxLight);
        ...
        // render microfacet diorama
            ...
            // choose a colour
            const float3 microfacetColour(0.64f); // steel
        ...
        // render final scene
            ...
            // red
            Material redLight; redLight.type = LIGHT; redLight.emission = float3(533.6f, 65.6f, 3.2f) / 2; redLight.Kd = float3(0.53360f, 0.06560f, 0.00320f);
            ...
            // green
            Material greenLight; greenLight.type = LIGHT; greenLight.emission = float3(116.0f, 307.2f, 62.4f) / 2; greenLight.Kd = float3(0.11600f, 0.30720f, 0.06240f);
            ...
            // white
            // Material whiteLight; whiteLight.type = LIGHT; whiteLight.emission = float3(884.0f, 780.8f, 29.6f) / 2; whiteLight.Kd = float3(0.88400f, 0.78080f, 0.02960f);
            ...
            // blue
            Material blueLight; blueLight.type = LIGHT; blueLight.emission = float3(116.0f, 62.4f, 307.2f) / 2; blueLight.Kd = float3(0.11600f, 0.06240f, 0.30720f);
            ...
            // black
            // Material blackLight; blackLight.type = MICROFACET; blackLight.Kd = float3(0.1f); blackLight.setRoughness(0.5f);
            ...
            // glass or metal orb
            Material glass; glass.type = GLASS; Material metal; metal.type = METAL; metal.Ks = float3(0.64f);
            Sphere orb = Sphere(float3(-0.5f, 1.0f, -14.5f), 4.0f, glass); globalScene.addBall(&orb);
